...
if (mysqli_num_rows($SQL_STATEMENT) > 0) {
    ...
    // Fallback: use legacy video_url/video_thumbnail from temples table if temple_videos is empty
    if (empty($templeVideos) && isset($Row->video_url) && !empty(trim($Row->video_url ?? ''))) {
        $templeVideos[] = (object)['video_url' => trim($Row->video_url), 'video_thumbnail' => $Row->video_thumbnail ?? null];
    }
                                    ...
                                    <div class="row mt-3 g-2 review-image zoom-gallery">
                                        ...
                                        ?>
                                                ...
                                                <div class="col-6">
                                                    ...
                                                    <a href="<?= $imagePath; ?>" id="image-<?= htmlspecialchars($image); ?>" class="gallery-overlay-hover dark-overlay position-relative d-block overflow-hidden rounded-3 gallery-image-link">



                                                        <img src="<?= $imagePath; ?>" alt="Gallery Image" class="rounded-3 gallery-thumb">
