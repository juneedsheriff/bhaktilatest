...
if (!empty($_REQUEST['del_t'])) {
    ...
    $query = "DELETE FROM `mantras_subcategory` WHERE `mantras_subcategory`.`index_id` = '$del_id'";
...
<div class="card-header position-relative">
    <div class="d-flex justify-content-between align-items-center flex-wrap">
        <div>
            <h6 class="fs-17 fw-semi-bold my-1">Sub Category </h6>
            <!-- <p class="mb-0">Temple Listing.</p> -->

        </div>
        <div class="text-end">
            <a href="add_mantras_subcategory.php" class="btn btn-primary fw-medium"><i class="fa-solid fa-plus me-1"></i>Add New God</a>
        </div>
    </div>
</div>
        ...
        <div class="card">
            <div class="card-body">
                <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        <?php $select = "SELECT * FROM `mantras_subcategory` WHERE index_id!='0' ORDER BY index_id DESC";
                        $SQL_STATEMENT = mysqli_query($DatabaseCo->dbLink, $select);
                        $num_rows = mysqli_num_rows($SQL_STATEMENT);
                        if ($num_rows != 0) {
                            $i = 1;
                            while ($Row = mysqli_fetch_object($SQL_STATEMENT)) {
                                $sql3 = mysqli_query($DatabaseCo->dbLink, "SELECT title FROM mantras_category WHERE index_id='" . $Row->categories_id . "'");
...
<?php } ?>
                                    ...
                                    <?php 
if ($user_role === 'Admin'): ?>
                                        <a class="btn btn-sm p-2 btn-primary text-white  edit-board alert-box-trigger waves-effect waves-light kill-drop" href="add_mantras_subcategory.php?id=<?php echo $Row->index_id; ?>"><i class="fas fa-pencil-alt"></i></a> &nbsp; &nbsp;
...
elseif ($user_role === 'Staff'): 
    if ($Row->status === 'unapproved'): ?>
       ...
       <a class="btn btn-sm p-2 btn-primary text-white edit-board alert-box-trigger waves-effect waves-light kill-drop" 
           href="add_mantras_subcategory.php.php?id=<?php echo $Row->index_id; ?>">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="header-title">Delete God </h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body" align="center">
                    <h5 class="text-center">Delete God Details?</h5>
                    <p>Are you sure you want to delete this God ? All data will be lost.</p>
                </div>
