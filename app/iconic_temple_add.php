...
if (isset($_REQUEST['submit'])) {
    ...
    if ($_REQUEST['id'] > 0) {
        $d_id = $_REQUEST['id'];
        $DatabaseCo->dbLink->query("UPDATE `iconic_temples` SET `title`='$title',`sthalam`='$sthalam',`puranam`='$puranam',`varnam`='$varnam',`highlights`='$highlights',`sevas`='$sevas',`speciality_title`='$speciality_title',`country`='$country',`state`='$state',`city`='$city',`address`='$address',`god_id`='$god_id',`my_stery`='$my_stery',`speciality`='$speciality',`time`='$time',`categories_id`='$categories_id',`status`='$status_1' WHERE `index_id`='$d_id'") or die(mysqli_error($DatabaseCo->dbLink));
    } else {
        $DatabaseCo->dbLink->query("INSERT INTO `iconic_temples`( `title`,`sthalam`, `puranam`,`varnam`,`highlights`,`sevas`,`speciality_title`,`country`,`state`,`city`,`address`,`god_id`,`my_stery`,`speciality`,`time`,`categories_id`,`status`) VALUES ( '$title','$sthalam', '$puranam','$varnam','$highlights','$sevas', '$speciality_title','$country','$state','$city','$address','$god_id','$my_stery','$speciality','$time','$categories_id','$status_1')") or die(mysqli_error($DatabaseCo->dbLink));
    ...
    if (!empty($_FILES['photos']['tmp_name']) && is_uploaded_file($_FILES['photos']['tmp_name'])) {
        $upload_image_photos = $uploadimage->upload($_FILES['photos'], "iconic_temple");
    }
    ...
    if (!empty($_FILES['banner']['tmp_name']) && is_uploaded_file($_FILES['banner']['tmp_name'])) {
        $upload_image_banner = $uploadimage->upload($_FILES['banner'], "iconic_temple/banner");
    }

    // Only update the database if there is a new image for photos
    if ($upload_image_photos != '') {
        $DatabaseCo->dbLink->query("UPDATE `iconic_temples` SET photos='$upload_image_photos' WHERE index_id='$d_id'");
    }

    // Only update the database if there is a new image for banner
    if ($upload_image_banner != '') {
        $DatabaseCo->dbLink->query("UPDATE `iconic_temples` SET banner='$upload_image_banner' WHERE index_id='$d_id'");
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $query = "SELECT gallery_image FROM `iconic_temples` WHERE index_id=?";
        ...
        if (isset($_FILES['gallery_image']) && !empty($_FILES['gallery_image']['name'][0])) {
            // Upload new images
            $uploadedImages = $imageUploader->uploadMultiple($_FILES['gallery_image'], 'iconic_temple/gallery');
        ...
        if (!empty($unique_images)) {
            ...
            $query = "UPDATE `iconic_temples` SET gallery_image=? WHERE index_id=?";
            $stmt = $DatabaseCo->dbLink->prepare($query);
    ...
    if ($_REQUEST['edit'] > 0) {
        header("location:iconic_temple_list.php?alt=1");
    } else {
        header("location:iconic_temple_add.php");
    }
...
if ($_REQUEST['id'] > 0) {
    // $titl = "Update ";
    $select = "SELECT * FROM iconic_temples WHERE index_id='" . $_REQUEST['id'] . "'"; //echo $select;
...
if ($imageIndex !== null && $nameIndex !== null) {
    ...
    $stmt = $dbLink->prepare("UPDATE iconic_temples SET gallery_image = REPLACE(gallery_image, ?, '') WHERE index_id = ?");
    ...
    if ($stmt->execute()) {
        ...
        $stmt = $dbLink->prepare("SELECT gallery_image FROM iconic_temples WHERE index_id = ?");
...
<div class="body-content">
    ...
    <div class="font-caveat fs-4 fw-bold fw-medium section-header__subtitle text-capitalize text-center text-primary text-xl-start mb-2"> Iconic Temple</div>
    <div class="row">
            ...
            <div class="card-body">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <p class="card-title-desc">Please fill the required iconic temples details.</p>
                        ...
                        <div class="control has-icons-left">
                          <input type="time" class="form-control" name="close_time" id="close_time" required="" placeholder="Temple Close Time" value="<?php echo date("H:i:s", strtotime($Row->close_time)); ?>">
                                        ...
                                        <div class="col-sm-4 mb-3">
                                            ...
                                            <label class="required fw-medium ">Category</label>
                                            ...
                                            <select class="form-select " name="categories_id" id="categories_id">
                                                ...
                                                // Make sure to use single quotes around the variable in the SQL query
                                                $Vselect = "SELECT * FROM iconic ORDER BY title";
                                                $VSQL_STATEMENT = mysqli_query($DatabaseCo->dbLink, $Vselect);
                                                while ($VRow = mysqli_fetch_object($VSQL_STATEMENT)) {
                                        ...
                                        <div class="col-sm-4 mb-3">
                                            ...
                                            <div class="field">
                                                <div class="control has-icons-left">
                                                    <input type="text" class="form-control" name="speciality_title" id="speciality_title" required="" placeholder="Enter Speciality title" value="<?php echo $Row->speciality_title; ?>">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-sm-4 mb-3">
                                            ...
                                            <div class="mb-2">
                                                ...
                                                <a href="./uploads/iconic_temple/<?php echo htmlspecialchars($Row->photos); ?>" class="d-inline-block" target="_blank">
                                                    <img src="./uploads/iconic_temple/<?php echo htmlspecialchars($Row->photos); ?>" alt="Featured" class="img-thumbnail" style="max-width:120px;max-height:120px;object-fit:contain;">
                                                </a>
                                        ...
                                        <div class="col-sm-4 mb-3">
                                            ...
                                            <div class="mb-2">
                                                <span class="d-block small text-muted mb-1">Current image:</span>
                                                <a href="./uploads/iconic_temple/banner/<?php echo htmlspecialchars($Row->banner); ?>" class="d-inline-block" target="_blank">
                                                    <img src="./uploads/iconic_temple/banner/<?php echo htmlspecialchars($Row->banner); ?>" alt="Banner" class="img-thumbnail" style="max-width:120px;max-height:120px;object-fit:contain;">
                                                </a>
                                                <p class="small text-muted mb-0 mt-1">Choose a new file below to replace.</p>
                                            </div>
                                            <?php endif; ?>
                      ...
                      <div class="field">
                        <div class="control has-icons-left">
                          <input type="number" class="form-control" name="order_by" id="order_by" required="" placeholder="Enter Order Number" value="<?php echo $Row->order_by; ?>">
                                        ...
                                        <div class="col-sm-4 mb-3 ">
                                            <!-- <label>Mystery  iconic_temples</label> -->
                                                ...
                                                <div class="row g-4">
                                                    ...
                                                    <div class="col-sm-6">
                                                        ...
                                                        <div class="">
                                                            ...
                                                            <select class="form-select mb-3" name="state" id="state">
                                                                ...
                                                                <?php
                                                                // Use saved country when editing so state list and selection are correct
                                                                $country_code = (isset($Row) && isset($Row->country) && $Row->country !== '') ? $Row->country : '';
                                                                // Load states based on the selected country
                                                                $state = isset($Row->state) ? $Row->state : ''; // Current state code
                                                                if ($country_code) {
                                                                    $stateQuery = "SELECT * FROM state WHERE country_code = '" . $DatabaseCo->dbLink->real_escape_string($country_code) . "' ORDER BY state_name";
                                                    ...
                                                    <div class="col-sm-6">
                                                        <div>
                                                            ...
                                                            <select class="form-select mb-3" name="city" id="city">
                                                                ...
                                                                <?php
                                                                // iconic_temples table uses "state" (state code), not state_code
                                                                $currentCity = isset($Row->city) ? $Row->city : ''; // Current city ID
                                                                $stateForCity = isset($Row->state) ? $Row->state : ''; // State code for filtering cities

                                                                // When editing, load cities (filter by state when available)
                                                                if (!empty($currentCity) || !empty($stateForCity)) {
                                                    ...
                                                    <div class="col-sm-6">
                                                        ...
                                                        <div class="">
                                                            ...
                                                            <textarea class="form-control" id="editor8" data-sample-short name="address" rows="5" placeholder="Enter Address"><?php echo $Row->address; ?></textarea>
                                            ...
                                            <div class="card-body">
                                                ...
                                                <?php
                                                    ...
                                                    foreach ($existingImages as $image) {
                                                        $imagePath = "uploads/iconic_temple/gallery" . htmlspecialchars($image);
                ...
                <div class="field">
                  <div class="control has-icons-left mt-30">
                    ...
                    <div class="mt-10" id=" iconic_templesname"></div>
                  </div>
                </div>
              </div> -->
...
<script>
    ...
    filebrowserUploadUrl: "/home/u196126541/domains/l360.online/public_html/bhaktikalpa/app/uploads/", // Change this to your actual backend URL
