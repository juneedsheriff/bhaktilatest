   ...
   // Ensure the database connection is included
    ...
    try {
        if ($action === 'update') {
            ...
            $sql = "UPDATE mantras_title SET title = ? WHERE index_id = ?";
            $stmt = $DatabaseCo->dbLink->prepare($sql);
            $stmt->bind_param("si", $name, $index_id);
        } elseif ($action === 'insert') {
            // Prepare insert statement
            $sql = "INSERT INTO mantras_title (title) VALUES (?)";
            $stmt = $DatabaseCo->dbLink->prepare($sql);
        ...
        if ($stmt->execute()) {
            $message = ($action === 'update') ? 'mantras updated successfully.' : 'mantras added successfully.';
