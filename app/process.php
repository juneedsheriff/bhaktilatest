...
header("Access-Control-Allow-Origin: https://m.bhaktikalpa.com");
...
if (isset($_POST['action']) && $_POST['action'] == 'login') {
    ...
    if ($username == 'bhaktikalpa') {
        ...
        // STAFF LOGIN (by email/username and password)
        $username_esc = $DatabaseCo->dbLink->real_escape_string($username);
        $password_enc = $DatabaseCo->dbLink->real_escape_string(base64_encode($password));

        $query = "SELECT * FROM staff 
                  WHERE LOWER(TRIM(username)) = LOWER('$username_esc') 
