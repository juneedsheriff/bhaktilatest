...
if(isset($_REQUEST['upload'])){
	...
	if(strtolower($ext) == "csv")
	{
        ...
        while (($getData = fgetcsv($file)) !== FALSE) {
	        if($i>0 && $getData[3] != ''){
           		$DatabaseCo->dbLink->query("INSERT INTO `iconic` (`banner`, `my_stery`, `god_id`, `title`, `description`, `small_description`, `photos`, `open_time`, `close_time`, `country`, `state`, `city`, `address`, `order_by`, `speciality`, `time`, `log_date`,`status`) VALUES ('$getData[0]','$getData[1]','$getData[2]','$getData[3]','$getData[4]','$getData[5]','$getData[6]','$getData[7]','$getData[8]','$getData[9]','$getData[10]','$getData[11]','$getData[12]','$getData[13]','$getData[14]','$getData[15]','$log_date','unapproved')");$j++;
	       	}$i++;
	   	}
	}
	header("location:iconic-category-upload.php?count=$j");
}
?>
<style>
    .icon-container {
...
<div class="body-content">
  ...
  <div class="font-caveat fs-4 fw-bold fw-medium section-header__subtitle text-capitalize text-center text-primary text-xl-start mb-2">Iconic Categories  - Bulk Upload</div>
  <div class="row">
    <div class="col-12">
        <div class="card">
            ...
            <?php }?>
        <div class="text-end">
            <a href="iconic-category.csv" class="btn btn-primary fw-medium mr-2 pr-2 mx-1"  download="iconic-category.csv" type="text/csv"><i class="fa-solid fa-download me-1"></i>Download Sample File</a>
        </div>
            <div class="card-body align-items-center" align="left">
										...
										<div class="d-flex position-relative my-1" align="center">	
											<form action="iconic-category-upload.php" method="post"  enctype="multipart/form-data">
