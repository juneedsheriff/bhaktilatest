...
if (!empty($_REQUEST['del_t'])) {
    ...
    $query = "DELETE FROM `iconic` WHERE `iconic`.`index_id` = '$del_id'";
    $query2 = $DatabaseCo->dbLink->query("DELETE FROM `iconic_temples` WHERE categories_id = '$del_id'");
...
<div class="card-header position-relative">
    <div class="d-flex justify-content-between align-items-center flex-wrap">
        <div>
            <h6 class="fs-17 fw-semi-bold my-1"> Iconic Homepages</h6>
            <!-- <p class="mb-0">Temple Listing.</p> -->

        </div>
        <div class="text-end">
            <a href="add-iconic-category.php" class="btn btn-primary fw-medium"><i class="fa-solid fa-plus me-1"></i>Add New Homepage</a>
        </div>
    </div>
</div>
        ...
        <div class="card">
            <div class="card-body">
                <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        <?php $select = "SELECT * FROM `iconic` WHERE index_id!='0' ORDER BY index_id DESC";
                        ...
                        ?>
                                <tr>
                                    <td><?php echo $Row->index_id;
                                        $i++; ?></td>
                                    <td>
                                        <?php if ($Row->photos != '') { ?>
                                            <a href="./uploads/iconic/<?php echo $Row->photos; ?>" target="_blank"><img src="./uploads/iconic/<?php echo $Row->photos; ?>" class=" header-profile-user" width="60" alt="" data-demo-src="./uploads/iconic/<?php echo $Row->photos; ?>"></a>
                                        <?php } ?>
                                    </td>
...
<?php } ?>
                                    ...
                                    <?php 
if ($user_role === 'Admin'): ?>
                                        <a class="btn btn-sm p-2 btn-primary text-white  edit-board alert-box-trigger waves-effect waves-light kill-drop" href="add-iconic-category.php?id=<?php echo $Row->index_id; ?>"><i class="fas fa-pencil-alt"></i></a> &nbsp; &nbsp;
...
elseif ($user_role === 'Staff'): 
    if ($Row->status === 'unapproved'): ?>
           ...
           href="add-iconic-category.php?id=<?php echo $Row->index_id; ?>">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="header-title">Delete Iconic Temple</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body" align="center">
                    <h5 class="text-center">Delete Iconic Temple Details?</h5>
                    <p>Are you sure you want to delete this Iconic Temple? All data will be lost.</p>
                </div>
