...
$staff_allowed = [];
if (!empty($Row->allowed_menus)) {
    $staff_allowed = array_map('trim', explode(',', $Row->allowed_menus));
}
function staff_has_access($key, $staff_allowed, $sidebar_menu_blocks) {
    if (in_array($key, $staff_allowed)) return true;
    if (isset($sidebar_menu_blocks[$key])) {
        $subs = $sidebar_menu_blocks[$key][1] ?? [];
        foreach ($subs as $item) {
            $page = is_array($item) ? $item[1] : $item;
            if (in_array($page, $staff_allowed)) return true;
        }
    }
    return false;
}
function staff_has_page($page, $staff_allowed, $sidebar_page_to_block) {
    if (in_array($page, $staff_allowed)) return true;
    $block = $sidebar_page_to_block[$page] ?? null;
    return $block && in_array($block, $staff_allowed);
}
                                            ...
                                            <div class="tab-content p-3">
                                                <div class="tab-pane active" id="home-1" role="tabpanel">
                                                    <div class="row">
                                                            ...
                                                            <div class="row mt-4">
                                                                <div class="col-12">
                                                                    <h5 class="header-title mb-3">Menu Access (Permissions)</h5>
                                                                    <p class="text-muted small mb-3">Sidebar menus and pages this staff can access.</p>
                                                                    <div class="border rounded p-3" style="max-height: 400px; overflow-y: auto;">
                                                                        <?php foreach ($sidebar_menu_blocks as $menu_key => $menu_data):
                                                                            ...
                                                                            $has_block = in_array($menu_key, $staff_allowed);
                                                                            $has_any = $has_block || staff_has_access($menu_key, $staff_allowed, $sidebar_menu_blocks);
                                                                            if (!$has_any) continue;
                                                                        ?>
                                                                        <div class="mb-3">
                                                                            <div class="fw-semibold text-dark">
                                                                                <i class="fa fa-check-circle text-success me-1"></i> <?php echo htmlspecialchars($block_label); ?>
                                                                            </div>
                                                                            <?php if (!empty($subs)): ?>
                                                                            <ul class="list-unstyled ms-3 mt-1 mb-0">
                                                                                <?php foreach ($subs as $item):
                                                                                    ...
                                                                                    $allowed = $has_block || staff_has_page($sub_page, $staff_allowed, $sidebar_page_to_block);
                                                                                    if (!$allowed) continue;
                                                                                ?>
                                                                                <li class="py-0"><i class="fa fa-check text-success me-1 small"></i> <?php echo htmlspecialchars($sub_label); ?></li>
                                                                                <?php endforeach; ?>
                                                                            </ul>
                                                                            <?php endif; ?>
                                                                        </div>
                                                                        <?php endforeach; ?>
                                                                        <?php if (empty($staff_allowed)): ?>
                                                                        <p class="text-muted mb-0">No menu access assigned.</p>
                                                                        <?php endif; ?>
                                                                    </div>
                                                                </div>
                                                            </div>
