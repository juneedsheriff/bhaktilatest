...
if (isset($_REQUEST['submit'])) {
  ...
  if ($_REQUEST['id'] > 0) {
    ...
    $DatabaseCo->dbLink->query("UPDATE `temples` SET `title`='$title', `temple_place`='$temple_place', `sthalam`='$sthalam',`puranam`='$puranam',`varnam`='$varnam',`highlights`='$highlights',`sevas`='$sevas',`open_time`='$open_time',`close_time`='$close_time',`country`='$country',`state`='$state',`city`='$city',`address`='$address',`log_date`='$log_date',`my_stery`='$my_stery',`god_id`='$god_id',`order_by`='$order_by',`speciality`='$speciality',`time`='$time',`status`='$status_1' WHERE `index_id`='$d_id'") or die(mysqli_error($DatabaseCo->dbLink));

  } else {

    $DatabaseCo->dbLink->query("INSERT INTO `temples`( `title`, `temple_place`, `sthalam`, `puranam`,`varnam`,`highlights`,`sevas`,`open_time`,`close_time`,`country`,`state`,`city`,`address`,`log_date`,`my_stery`,`god_id`,`order_by`,`speciality`,`time`,`status`) VALUES ( '$title', '$temple_place', '$sthalam', '$puranam','$varnam','$highlights','$sevas','$open_time','$close_time','$country','$state','$city','$address','$log_date','$my_stery','$god_id','$order_by','$speciality','$time','$status_1')") or die(mysqli_error($DatabaseCo->dbLink));
  ...
  if ($_REQUEST['edit'] > 0) {
    ...
    header("location:list-temple.php");
...
<div class="body-content">
  ...
  <div class="font-caveat fs-4 fw-bold fw-medium section-header__subtitle text-capitalize text-center text-primary text-xl-start mb-2">Temple temples</div>
...
<script>
  ...
  $('.remove').click(function() {
    ...
    $.ajax({
      ...
      url: "list-temple.php", // Server endpoint to handle image removal
